...
export default function OwnerCustomerDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!customer) return <div className="px-6 py-16 text-[#8a94a3]">{t("common.loading")}</div>;
  ...
  return (
    <div className="mx-auto max-w-3xl px-6 py-10">
      ...
      <h2 className="mt-8 mb-2 text-xs font-semibold uppercase text-[#8a94a3]">{t("nav.vehicles")}</h2>
      <div className="flex flex-wrap gap-2">
        {customer.vehicles.map((v) => (
          <span key={v.id} className="rounded-full border border-[#263041] px-3 py-1 text-sm">
            {v.make} {v.model} ({v.year})
          </span>
        ))}
      </div>

      <h2 className="mt-8 mb-2 text-xs font-semibold uppercase text-[#8a94a3]">{t("nav.appointments")}</h2>
      <div className="flex flex-col gap-2">
        {customer.appointments.map((a) => (
          <div key={a.id} className="flex items-center justify-between rounded-lg border border-[#263041] bg-[#151b25] p-3 text-sm">
            <span>
        ...
        {customer.appointments.length === 0 && <p className="text-[#8a94a3]">{t("common.noData")}</p>}
      </div>

      <h2 className="mt-8 mb-2 text-xs font-semibold uppercase text-[#8a94a3]">{t("nav.requests")}</h2>
      <div className="flex flex-col gap-2">
        {customer.requests.map((r) => (
          <div key={r.id} className="flex items-center justify-between rounded-lg border border-[#263041] bg-[#151b25] p-3 text-sm">
            <span>
