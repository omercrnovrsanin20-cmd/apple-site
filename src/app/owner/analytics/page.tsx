  ...
  if (!data.hasData) {
    return (
      <div className="mx-auto max-w-4xl px-6 py-16">
        ...
        <p className="mt-8 rounded-lg border border-dashed border-[#263041] p-8 text-center text-[#8a94a3]">
          {t("owner.noDataYet")}
        </p>
      </div>
    );
  ...
  return (
    <div className="mx-auto max-w-4xl px-6 py-10">
      ...
      <Section title={t("owner.revenueOverTime")}>
        <MiniLineChart data={data.revenueOverTime} color="#4fd1c5" />
      </Section>

      <Section title={t("owner.jobsOverTime")}>
        <MiniLineChart data={data.jobsOverTime} color="#8b7cf6" />
      </Section>
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="mt-8">
      <h2 className="mb-3 text-xs font-semibold uppercase tracking-wide text-[#8a94a3]">{title}</h2>
      <div className="rounded-lg border border-[#263041] bg-[#151b25] p-5">{children}</div>
    </div>
  );
}

function Empty({ text }: { text: string }) {
  return <p className="text-sm text-[#8a94a3]">{text}</p>;
}
